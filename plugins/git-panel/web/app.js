...
function renderFiles(){$('groups').replaceChildren();for(const [title,isStaged,filter] of [['已暂存的更改',true,staged],['更改',false,changed]]){const list=state.files.filter(filter);if(!list.length)continue;const d=el('details');d.open=true;d.append(el('summary',`${title} (${list.length})`));for(const f of list){const row=el('div',undefined,'file');if(selected?.path===f.path&&selected?.staged===isStaged)row.classList.add('selected');const name=el('button',f.path,'filename');name.title=f.path;name.onclick=()=>showDiff({path:f.path,staged:isStaged});row.append(name);if(!isStaged&&f.x!=='?'){const discard=el('button','↶','action');discard.title='放弃未暂存更改';discard.setAttribute('aria-label','放弃 '+f.path+' 的未暂存更改');discard.onclick=()=>{if(confirm(`放弃 ${f.path} 的未暂存修改？\n此操作不能通过 Git 撤销。`))action('discard',{path:f.path,confirm:true});};row.append(discard);}const b=el('button',isStaged?'−':'+','action');b.title=isStaged?'取消暂存':'暂存';b.setAttribute('aria-label',b.title+' '+f.path);b.onclick=()=>action(isStaged?'unstage':'stage',{path:f.path});row.append(b,el('span',isStaged?f.x:f.y===' '?'M':f.y,'code'));d.append(row);}$('groups').append(d);}if(!state.files.length)$('groups').append(el('p','✓ 工作区干净','muted'));}
...
async function showDiff(selection){
 ...
 $('diffTitle').textContent=selection.title||selection.path;
...
const mq=matchMedia('(prefers-color-scheme:light)');function theme(){document.body.classList.toggle('light',$('theme').value==='light'||$('theme').value==='auto'&&mq.matches);localStorage.setItem('git-theme',$('theme').value);}$('theme').value=localStorage.getItem('git-theme')||'dark';$('theme').onchange=theme;mq.onchange=theme;theme();
